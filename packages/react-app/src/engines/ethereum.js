...
export async function signEthereumRequests(payload, state, setState) {
  ...
  if (connector) {
    if (!state.walletcontroller.isActive()) {
      await state.walletcontroller.init(activeIndex, chainId);
    }
    ...
    switch (payload.method) {
      case "eth_sendTransaction":
        ...
        if (address.toLowerCase() === addressRequested.toLowerCase()) {
          result = await state.walletcontroller.sendTransaction(transaction);
        } else {
      ...
      case "eth_signTransaction":
        ...
        addressRequested = transaction.from;
        if (address.toLowerCase() === addressRequested.toLowerCase()) {
          result = await state.walletcontroller.signTransaction(transaction);
        } else {
          errorMsg = "Address requested does not match active account";
        }
        break;
      case "eth_sign":
        ...
        if (address.toLowerCase() === addressRequested.toLowerCase()) {
          result = await state.walletcontroller.signMessage(dataToSign);
        } else {
          errorMsg = "Address requested does not match active account";
        }
        break;
      case "personal_sign":
        ...
        if (address.toLowerCase() === addressRequested.toLowerCase()) {
          result = await state.walletcontroller.signPersonalMessage(dataToSign);
        } else {
          errorMsg = "Address requested does not match active account";
        }
        break;
      case "eth_signTypedData":
        ...
        addressRequested = payload.params[0];
        if (address.toLowerCase() === addressRequested.toLowerCase()) {
          result = await state.walletcontroller.signTypedData(dataToSign);
        } else {
          errorMsg = "Address requested does not match active account";
        }
        break;
