...
async function deployUpgradable(
  ...
  const deployment = await deployments.deploy(artifactName, {
    ...
    proxy: {
      ...
      execute: {
        init: { methodName: "initialize", args: initializeFunctionArgs },
      },
    },
  ...
  try {
    const implementationDeployment = await ethers.getContractAt(
      contractName,
      implementation
    );
    const tx = await implementationDeployment.initialize(
      ...initializeFunctionArgs
    );
    await tx.wait();
