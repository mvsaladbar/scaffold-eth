...
/* Notes section
  Deposit cost : 0.0195798 ETH (deposit with LP)
  Withdraw cost : 0.0333284 ETH (withdraw)
  Claim cost : not available
*/

describe("Yearn USDC - Tests mainnet fork", async () => {
  let binanceWallet;
  beforeEach(async () => {
    await fork_network();
  ...
  it("should test initial parameters", async () => {
    const tokenIn = await yearnUsdcStrategy.tokenIn();
    const yearnVault = await yearnUsdcStrategy.yearnVault();
    const managerContainer = await yearnUsdcStrategy.managerContainer();
    expect(tokenIn.toLowerCase(), "✖️ Token not right").to.eq(
      USDC_ADDRESS.toLowerCase()
    );
    expect(yearnVault.toLowerCase(), "✖️ Vault not right").to.eq(
      "0xa354f35829ae975e850e23e9615b11da1b3dc4de".toLowerCase()
    );
    expect(
  ...
  it("should test get rewards for non-participant", async () => {
    const result = await yearnUsdcStrategy.getRewards(tokensHolder.address);
    expect(result, "✖️ Rewards not right").to.eq(0);
  });

  it("should test multiple deposits", async () => {
    const balanceUsdcAmount = 20000000000;
    const depositUsdcAmount = 10000000000;
    binanceWallet = await ethers.getSigner(BINANCE_WALLET_ADDRESS);

    const rewardsBeforeFirstDeposit = await yearnUsdcStrategy.getRewards(
    ...
    await usdc
      ...
      .transfer(tokensHolder.address, balanceUsdcAmount);

    await usdc
      .connect(tokensHolder)
      .approve(fullHoldingFactoryContract.address, balanceUsdcAmount);
    const depositTx = await fullHoldingFactoryContract
      .connect(tokensHolder)
      .deposit(USDC_ADDRESS, balanceUsdcAmount);
    await depositTx.wait();

    const collateralBeforeFirstDeposit =
      await fullShareRegistryContract.collateral(fullHoldingContract.address);
    expect(
      collateralBeforeFirstDeposit,
      "✖️ Collateral before first deposit not right"
    ).to.eq(balanceUsdcAmount);

    let [evHolding, evSender, evToken, evStrategy, evAmount, evResult] =
      await _deposit(depositUsdcAmount);
    ...
    const rewardsAfterFirstDeposit = await yearnUsdcStrategy.getRewards(
      tokensHolder.address
    );
    console.log(
    ...
    const depositInfo = await yearnUsdcStrategy.recipients(
      fullHoldingContract.address
    );
    const rewardsAfterSecondDeposit = await yearnUsdcStrategy.getRewards(
      tokensHolder.address
    );
    console.log(
  ...
  it("should withdraw", async () => {
    ...
    let depositInfo = await yearnUsdcStrategy.recipients(
      fullHoldingContract.address
    );

    const collateralBeforeWithdraw = await fullShareRegistryContract.collateral(
      fullHoldingContract.address
    );
    expect(
    ...
    const balanceOfHoldingAfterFirstWithdrawal = await usdc.balanceOf(
      fullHoldingContract.address
    );
    console.log(
    ...
    depositInfo = await yearnUsdcStrategy.recipients(
      fullHoldingContract.address
    );
    ...
    const balanceOfHolding = await usdc.balanceOf(fullHoldingContract.address);
...
async function _deposit(amount) {
  const investTx = await fullStrategyManager
    .connect(tokensHolder)
    .invest(USDC_ADDRESS, yearnUsdcStrategy.address, amount, EMPTY_BYTES);
