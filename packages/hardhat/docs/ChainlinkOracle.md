...
```solidity
function _get(address multiply, address divide, uint256 decimals) external view returns (uint256)
```
...
|---|---|---|
| multiply | address | undefined
| divide | address | undefined
...
```solidity
function getDataParameter(address multiply, address divide, uint256 decimals) external pure returns (bytes)
```
