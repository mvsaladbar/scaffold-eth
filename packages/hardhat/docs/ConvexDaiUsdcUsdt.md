...
> ConvexDaiUsdcUsdt strategy used for cDai+cUsdc+cUsdt
...
| _recipient | address | on behalf of
| _data | bytes | extra data returns receipt tokens amount/obtained shares returns returned token in amount
...
```solidity
function lpGetter() external view returns (contract ICurveDaiUsdcUsdtLPGetter)
```
