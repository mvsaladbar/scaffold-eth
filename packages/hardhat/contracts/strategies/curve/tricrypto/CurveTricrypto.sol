...
import "./CurveTricryptoBase.sol";

/// @title CurveTricrypto strategy
/// @author Cosmin Grigore (@gcosmintech)
contract CurveTricrypto is IStrategy, CurveTricryptoBase {
    ...
    constructor(
        ...
        lpVault = ICurveTricryptoLiquidityPool(_lpVault);
        lpGauge = ICurveTricryptoLPGauge(_gauge);
        curveMinter = ICurveMinter(_minter);

        curveLPGetter = ICurveTricryptoLPGetter(_curveLpGetter);

        tokenOut = address(0);
        tokenIn = lpVault.token();
        rewardToken = lpGauge.crv_token();
    }

    /// @notice sets the new address
    /// @param _newAddr the new address
    function setTricryptoGauge(address _newAddr)
        external
        onlyValidAddress(_newAddr)
        onlyOwner
    {
        emit GaugeUpdated(address(lpGauge), _newAddr);
        lpGauge = ICurveTricryptoLPGauge(_newAddr);
        rewardToken = lpGauge.crv_token();
    }
